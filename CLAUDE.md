...
`localStorage`의 `codingvillage-stamps`에 넣습니다. 서버로 보내지 않습니다.
같은 폰으로 다시 들어오면 그대로 남아 있고, 다른 기기에서는 처음부터입니다.

전시 부스에서 태블릿 하나를 여러 사람이 돌려 쓰면 다음 손님이 앞사람 도장을 보게 되므로,
도장판 칸 맨 아래에 **「도장판 비우고 처음부터」** 를 두었습니다.
한창 도는 중에 잘못 눌리면 그때까지 걸은 게 사라지니, **한 번 물어본 뒤** 지웁니다.
지우면 방명록 안내 띠도 다시 뜹니다.
